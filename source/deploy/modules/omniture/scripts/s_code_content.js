...
var s_account="pfizerpronutrientsdev" // Dev 
...
s.linkInternalFilters="javascript:,mailto:,advil.com"
...
s.siteID="Pronutrients"            // leftmost value in pagename
s.defaultPage="Default.aspx"       // filename to add when none exists
...
s.pathExcludeList=""   // elements to exclude from the path
  ...
  /*function for sanitising page title*/
...
function s_doPlugins(s) {
  ...
  /* Set PageName and Hierarcy */	
  tempSCPage=s.getPageName();
  if(!s.hier1) s.hier1=s.channelExtract("/",10,tempSCPage,1).toLowerCase();
  //if(!s.pageName && !s.pageType) s.pageName=s.hier1 + SCStrip(sanitizevalue(document.title));
  if(!s.pageName && !s.pageType) 
  {
    var strPageName = ""
    if (document.title.toLowerCase() == "product labeling")
    {
        var strParentURL = top.location.href;
        var strprodName = strParentURL.substring(strParentURL.lastIndexOf('/')+1, strParentURL.lastIndexOf('.'));
        strPageName = s.hier1 + strprodName + "/" + SCStrip(sanitizevalue(document.title));
        s.pageName = strPageName.toLowerCase();
        //s.pageName=s.hier1 + SCStrip(sanitizevalue(document.title));        
    }
    else
    {
        strPageName = s.hier1 + SCStrip(sanitizevalue(document.title));
        s.pageName=strPageName.toLowerCase();
    }
...
/* External Campaigns */
	if(!s.campaign)
		s.campaign=s.getQueryParam('source,HBX_PK,HBX_OU,o',':');
	...
	if(s.url){
		 //Track FileName
                 	s.eVar7=s.prop7=s.url.substring(s.url.lastIndexOf("/")+1,s.url.length).toLowerCase();
	                s.prop8=s.pageName;
  ...
  /* Timeparting EST */
  s.prop15=s.eVar15=s.getTimeParting('h','-5','2011'); // Set hour 
  s.prop16=s.eVar16=s.getTimeParting('d','-5','2011'); // Set day
  s.prop17=s.eVar17=s.getTimeParting('w','-5','2011'); // Set Weekend / Weekday

   
  if(top.location.pathname.toLowerCase().indexOf("error.aspx") > -1)
  ...
  /* getNewRepeat 1.0 */
	s.prop18=s.eVar18=s.getNewRepeat();
	    
 /* Page URL*/
	if(s.pageName)
        s.prop23=s.eVar23=document.URL.toLowerCase();
    
    
  
	var pageURL = top.location.href;
	var pathname = top.location.pathname;
	if (pathname.toLowerCase().indexOf("default.aspx") > -1 || pathname == "/")
	{
		s.channel = "home page";
		s.prop1 = s.eVar1 = "advil/home page";
	    s.events=s.apl(s.events,"event5",",",2)
	}
	else if(pageURL.toLowerCase().indexOf("/ourproducts/") > -1 || pageURL.toLowerCase().indexOf("producthub.aspx") > -1)
	{
		s.channel = "our products";
		s.prop1 = s.eVar1 = "advil/our products";
	    s.events=s.apl(s.events,"event6",",",2)
	}
	else if(pageURL.toLowerCase().indexOf("/informationcenter/") > -1 || pageURL.toLowerCase().indexOf("informationcenter.aspx") > -1)
	{
		s.channel = "information center";
		s.prop1 = s.eVar1 = "advil/information center";
    }
	else if(pageURL.toLowerCase().indexOf("faq.aspx") > -1)
	{
		s.channel = "faq";
		s.prop1 = s.eVar1 = "advil/faq";
	}
	else if(pageURL.toLowerCase().indexOf("contact.aspx") > -1)
	{
		s.channel = "contact us";
		s.prop1 = s.eVar1 = "advil/contact us";
	}
	else if(pageURL.toLowerCase().indexOf("buynow.aspx") > -1)
	{	
	    s.channel = "buy now";
	    s.prop1 = s.eVar1 = "advil/buy now";
	}
	else if(pageURL.toLowerCase().indexOf("communications.aspx") > -1)
	{
		s.channel = "advil communications";
		s.prop1 = s.eVar1 = "advil/advil communications";
	}
	if(pageURL.toLowerCase().indexOf("/ourproducts/") > -1)
	{
	    s.events=s.apl(s.events,"prodView",",",2)
	    s.products="Product;" + pageURL.substring(pageURL.lastIndexOf('/')+1, pageURL.lastIndexOf('.')).toLowerCase();
	    s.prop14 = s.eVar14 = pageURL.substring(pageURL.lastIndexOf('/')+1, pageURL.lastIndexOf('.')).toLowerCase();
	}
...
{
    var pathname = top.location.pathname;
    if (pathname.toLowerCase().indexOf("communications.aspx") > -1)
    {
    var isRegistered = document.getElementById('ctl00_AdvilBodyContent_ucNewsletterSignUp_hdnIsRegistered').value;
	if(isRegistered == 'Yes')
	{
        document.getElementById('ctl00_AdvilBodyContent_ucNewsletterSignUp_hdnIsRegistered').value = "No";
        var s=s_gi(s_account);
        s.linkTrackVars='events';
        s.linkTrackEvents='event3';
        s.events='event3';
    	var linktitle = "Sign-up";
    	var frndlypagename = s.pageName;
        s.tl(linktitle,'o',frndlypagename);
...
function linktitletrack(obj,linktitle,linkpos,_event,setmoduletracking,linktype,navigateurl)
{   
    var s=s_gi(s_account);
    s.linkTrackVars='events,prop38,prop39,prop40,prop41,eVar38,eVar39,eVar40,eVar41,prop3,eVar3,prop4,eVar4,prop5,eVar5,prop6,eVar6,eVar20,eVar9';
    s.linkTrackEvents='None';
	s.events='';
    if (_event != undefined && _event != null && _event != '')
    {
        s.linkTrackEvents=_event;
	    s.events=s.apl(s.events,_event,",",2)
	}
	if (linktitle == undefined || linktitle == null || linktitle == "")
    {
        var linktitle = obj.title;    
        if (linktitle == undefined || linktitle == null || linktitle == "")
        linktitle = obj.innerText;        
        if (linktitle == undefined || linktitle == null || linktitle == "")
        linktitle = obj.href;
    }	
    if(linktitle)
    linktitle = sanitizevalue(linktitle);
    else
    return;
    frndlypagename = s.pageName.toLowerCase();
    linktitle = linktitle.toLowerCase();
    linkpos = linkpos.toLowerCase();
    s.prop38 = s.eVar38 = frndlypagename;
    s.prop39 = s.eVar39 = linktitle;
    s.prop40 = s.eVar40 = frndlypagename + " : " + linktitle;
    s.prop41 = s.eVar41 = linkpos;
//    var strpageName = s.channel;
//    if (strpageName == undefined || strpageName == null || strpageName == "")
//    {
//        strpageName = s.pageName.substr(s.pageName.lastIndexOf('/') + 1)
//    }
    if (setmoduletracking == 1)
    {
        var strModuleTrack = "";
        if(linkpos != undefined && linkpos != null && linkpos   != "")
            strModuleTrack = frndlypagename + " : " + linkpos + " : " + linktitle; 
        else
            strModuleTrack = frndlypagename + " : " + linktitle; 
        
        s.prop3 = s.eVar3 = strModuleTrack.toLowerCase();
    }
    if(linktype != undefined && linktype != null)
    {
        ...
        {
            s.eVar20 = s.pageName;
            s.prop6 = s.eVar6 = frndlypagename + " : health care professionals click here" //Custom Link
        }
        else if (linktype == 'partnersite')
        {
            var pageURL = top.location.href;
	        var pathname = top.location.pathname;
	        var strPartnerSite = "";
	        if (pathname.toLowerCase().indexOf("default.aspx") > -1 || pathname == "/")
	            strPartnerSite = frndlypagename + " : Thermacare Module : Learn More";
            else if(pageURL.toLowerCase().indexOf("/ourproducts/") > -1 || pageURL.toLowerCase().indexOf("producthub.aspx") > -1)
	            strPartnerSite = frndlypagename + " : Thermacare Module : Learn More";
	        
	        s.prop4 = s.eVar4 = strPartnerSite.toLowerCase();    
        }
        else if (linktype == 'navigation')
        {
            var strNavigation = frndlypagename + " : " + "Nav" + " : " + linktitle;
            s.prop5 = s.eVar5 = strNavigation.toLowerCase();
            if (strNavigation.toLowerCase().indexOf("advil communications") > -1) //Custom Link
                s.prop6 = s.eVar6 = frndlypagename + " : " + linktitle;
            
        }
        else if (linktype == 'wholesaler')
            s.eVar9 = s.pageName;
     }
  
    if (obj != null)   
        s.tl(true,'o',frndlypagename);
        
    s.prop38 = s.prop39 = s.prop40 = s.prop41 = s.eVar38 = s.eVar39 = s.eVar40 = s.eVar41 = s.prop3 = s.eVar3 = s.prop4 = s.eVar4 = s.prop5 = s.eVar5 = s.eVar20 = s.eVar9 = "";
	s.linkTrackVars='None';
    s.linkTrackEvents='None';
    s.events = '';
    if(navigateurl != undefined && navigateurl != '' && navigateurl.indexOf(':') < 0)
    {
        location.pathname = navigateurl;
    }
    else if (navigateurl != undefined && navigateurl != '')
    {
        var navigateurl = navigateurl.substr(navigateurl.indexOf(":") + 1);
        eval(navigateurl);
    }
}
...
function setSubSection(obj)
{
    var s=s_gi(s_account);
    var pageURL = top.location.href;
	s.linkTrackVars='prop2,eVar2,prop6,eVar6';
    s.linkTrackEvents='None';
    frndlypagename = s.pageName;
    if(pageURL.toLowerCase().indexOf("informationcenter.aspx") > -1)
    {
        var strIC = "Advil/Information Center/" + obj.innerText.replace(/[^a-zA-Z 0-9,&]+/g,'');
        s.prop2 = s.eVar2 = strIC.toLowerCase();
    }
	else if(pageURL.toLowerCase().indexOf("faq.aspx") > -1)
	{
	    var strFAQ = obj.innerText.replace(/[^a-zA-Z 0-9,&]+/g,'');
	    s.prop2 = s.eVar2 = "advil/faq/" + strFAQ.toLowerCase();
	    s.prop6 = s.eVar6 = s.pageName + " : " + strFAQ.toLowerCase(); //Custom Link
	}
	else // Our Products Subsection
	{
	    var strProduct = obj.title.replace(/[^a-zA-Z 0-9,&]+/g,'');
	    strProduct = strProduct.replace("Hello",""); // 'Advil® Hello' title for Advil
		s.prop2 = s.eVar2 = "advil/ourproducts/" + strProduct.toLowerCase();
	}
	s.tl(obj,'o',frndlypagename);
	s.prop2 = s.eVar2 = s.prop6 = s.eVar6 = "";
	s.linkTrackVars='prop38,prop39,prop40,prop41';
    s.linkTrackEvents='None';
}
function setrecalllink(obj)
{
    var s=s_gi(s_account);
    s.linkTrackVars='prop3,eVar3';
    s.linkTrackEvents='None';
    var strRecall = s.pageName + " : Recall";
    s.prop3 = s.eVar3 = strRecall.toLowerCase();
    s.tl(obj,'o','Recall');
    s.prop3 = s.eVar3 = '';
    s.linkTrackVars='prop38,prop39,prop40,prop41';
    s.linkTrackEvents='None';
...
function setSavingsForYou(productname,url,linkname)
{
    var s=s_gi(s_account);
    var pageURL = top.location.href;
	s.linkTrackVars='events,prop3,eVar3,prop4,eVar4';
    s.linkTrackEvents='event11';
	s.events=s.apl(s.events,"event11",",",2)
	var strPageName = "";
	var strSavingsforYou = "";
    var pageURL = top.location.href;
	var pathname = top.location.pathname;
	if (pathname.toLowerCase().indexOf("default.aspx") > -1 || pathname == "/")
		strPageName = "Home Page";
	else if(pageURL.toLowerCase().indexOf("/ourproducts/") > -1 || pageURL.toLowerCase().indexOf("producthub.aspx") > -1)
		strPageName = "Our Products";
	else if(pageURL.toLowerCase().indexOf("buynow.aspx") > -1)
		strPageName = "Buy Now";

    if (linkname == "")
	    strSavingsforYou = s.pageName + " : Savings for You : " +  productname + " : Get Coupon"
    else 
        strSavingsforYou = s.pageName + " : " + productname + " : Available Coupons : " +  linkname
    //s.prop4 = s.eVar4 = url
    s.prop3 = s.eVar3 = s.prop4 = s.eVar4 = strSavingsforYou.toLowerCase();
    s.tl(strPageName,'o',productname);
    s.prop3 = s.eVar3 = s.prop4 = s.eVar4 = "";
	s.linkTrackVars='prop38,prop39,prop40,prop41';
	s.linkTrackEvents='None';
	s.events = '';
}
...
s.Media.playerName="AdobeFlashPlayer"
...
s.visitorNamespace="pfizer"
/* Uncomment the two lines below once FPC are setup.
* s.trackingServer="metrics.pfizer.com"
* s.trackingServerSecure="smetrics.pfizer.com"
*/
s.dc=122
